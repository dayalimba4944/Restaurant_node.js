...
exports.store = (req, res) => {
    try {
        ...
        if (!prise) {
            errors.prise = 'Prise URL is required';
        }
        ...
        const query = 'INSERT INTO events (title, description, image_url , prise , status ) VALUES (?, ?, ? , ? , ?)';
        const values = [title, description, img , prise , status];
...
exports.update = (req, res) => {
    try {
        ...
        executeQuery(checkQuery, checkValues, (error, results) => {
            ...
            // Update the item
            const updateQuery = 'UPDATE events SET title = ?, description = ?, image_url = ?, prise = ? , status = ? WHERE id = ?';
            const updateValues = [title, description, img, prise , status, id];
